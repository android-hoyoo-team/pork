        ...
        protected override bool Execute(string name, CefV8Value obj, CefV8Value[] arguments, out CefV8Value returnValue, out string exception)
        {
            ...
            switch (name)
            {
                case "CallCSharpMethod":
                    ...
                    {
                        methodName = arguments[0].GetStringValue();
                        objArray = new object[arguments.Length-1];
                        for (int i = 1; i < arguments.Length; i++)
                        { 
                                objArray[i-1]=arguments[i].GetStringValue();
                        }
                        //string values = arguments[1].GetStringValue();
                    }
                    string res = Global.CallMethod(activeBrowser, methodName, objArray);
